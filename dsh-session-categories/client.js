window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		//#region session-categories extension
		const SC_API = "/api/dsh-session-categories";
		/** GET：读取某工作区的分类 store。 */
		async function scFetchStore(workspacePath) {
			const res = await fetch(SC_API + "?ws=" + encodeURIComponent(workspacePath));
			return await res.json();
		}
		/** POST：执行分类操作（create/rename/delete/assign）。 */
		async function scMutate(payload) {
		...
		const scCssTag = "dsh-session-categories/cat.css";
		const scCss = `
.sc-folder { display:flex; flex-direction:column; gap:2px; margin:0 0 5px 18px; transition:margin-top .18s ease; }
.sc-folderRow { display:flex; align-items:center; gap:6px; padding:5px 10px; min-height:28px; border-radius:8px; cursor:pointer; color:var(--dsw-alias-label-primary); user-select:none; }
.sc-folderRow:hover { background:var(--dsw-alias-interactive-bg-hover); }
.sc-folderRow.dragover { outline:1px dashed var(--dsw-alias-brand-primary); outline-offset:-1px; }
.sc-folderRow.sc-dragging { opacity:.4; }
.sc-folder.sc-openSlot { position:relative; margin-top:26px; }
.sc-folder.sc-openSlot::before { content:""; position:absolute; top:-2px; left:8px; right:8px; height:2px; border-radius:2px; background:var(--dsw-alias-brand-primary); pointer-events:none; }
.sc-folderName { flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:13px; font-weight:500; }
.sc-folderCount { font-size:11px; line-height:16px; color:var(--dsw-alias-label-tertiary); flex-shrink:0; background:var(--dsw-alias-interactive-bg-hover); padding:0 6px; border-radius:999px; }
.sc-folderOps { display:none; gap:2px; }
.sc-folderRow:hover .sc-folderOps { display:flex; }
.sc-folderOps button { width:20px; height:20px; border-radius:6px; }
.sc-newcat { display:flex; align-items:center; gap:6px; padding:4px 10px; margin:2px 0 5px 18px; color:var(--dsw-alias-label-secondary); font-size:13px; border:none; border-radius:6px; background:transparent; cursor:pointer; }
.sc-newcat:hover { color:var(--dsw-alias-label-primary); background:var(--dsw-alias-interactive-bg-hover); }
.sc-newcat input { flex:1; min-width:0; min-height:26px; border:1px solid var(--dsw-alias-border-l2); border-radius:6px; background:var(--dsw-alias-bg-base); color:var(--dsw-alias-label-primary); padding:2px 8px; font-size:13px; outline:none; }
.sc-newcat input:focus { border-color:var(--dsw-alias-brand-primary); }
.sc-newcat-x { display:inline-flex; flex:none; align-items:center; justify-content:center; width:20px; height:20px; padding:0; border:none; border-radius:6px; background:transparent; color:var(--dsw-alias-label-tertiary); cursor:pointer; }
.sc-newcat-x:hover { color:var(--dsw-alias-label-primary); background:var(--dsw-alias-interactive-bg-hover); }
.sc-error { margin:4px 10px 8px; padding:6px 10px; border-radius:8px; border:1px solid var(--dsw-alias-state-error-primary); color:var(--dsw-alias-state-error-primary); font-size:12px; line-height:1.5; white-space:pre-wrap; }
`;
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(scCssTag) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-session-categories";
			tag.dataset.pluginCss = scCssTag;
			tag.textContent = scCss;
			document.head.appendChild(tag);
		}
		/**
		* 分类树：每个工作区 → 分类文件夹 + 未分类文件夹 → 会话行。
		* 复用官方 ProjectRowItem（工作区头，含重命名/删除/新建会话）与
		* SessionNodeItem（会话行，含重命名/fork/归档菜单与 hover 卡），
		* 数据经 webServer API 持久化；拖拽会话到文件夹 = 移动分类。
		* 工作区与文件夹默认展开（折叠状态为本地 UI 状态）。
		*/
		function CategoryTree({ useSessions, onSessionRename, onSessionArchive, forkSession, workspaces, archivedSessionIds, startSession, connectWorkspace, open, onRenameRequest, onDeleteRequest, t }) {
			const byId = useSessions((state) => state.byId);
			const current = useSessions((state) => state.current);
			const archived = new Set(archivedSessionIds);
			const descendants = (0, _deepseek_ai_dsh_client_runtime_client.indexSubagentDescendants)(byId);
			const [stores, setStores] = (0, react.useState)({});
			const [loadedWs, setLoadedWs] = (0, react.useState)({});
			const [error, setError] = (0, react.useState)("");
			const [dragId, setDragId] = (0, react.useState)(null);
			const [collapsedWs, setCollapsedWs] = (0, react.useState)({});
			const [collapsed, setCollapsed] = (0, react.useState)({});
			const [creatingFor, setCreatingFor] = (0, react.useState)(null);
			...
			const [dragCat, setDragCat] = (0, react.useState)(null);
			const [catMarker, setCatMarker] = (0, react.useState)(null);
			const [now, setNow] = (0, react.useState)(Date.now());
			(0, react.useEffect)(() => {
				const timer = window.setInterval(() => setNow(Date.now()), 30000);
				return () => window.clearInterval(timer);
			}, []);
			const reportError = (msg) => setError((s) => (s ? s + "\n" : "") + msg);
			const load = async (wsPath) => {
				try {
					const res = await scFetchStore(wsPath);
					if (res && !res.error) {
						setStores((m) => Object.assign({}, m, { [wsPath]: res }));
						setLoadedWs((l) => Object.assign({}, l, { [wsPath]: true }));
					} else if (res && res.error) {
						reportError("sc.get(" + wsPath + "): " + res.error);
					}
				} catch (err) {
					reportError("sc.get threw: " + String((err && err.message) || err));
				}
			};
			(0, react.useEffect)(() => {
				for (const ws of workspaces) {
					if (!loadedWs[ws.path]) load(ws.path);
				}
			}, [workspaces]);
			const mutate = async (wsPath, action, extra) => {
				try {
					const res = await scMutate(Object.assign({ action, ws: wsPath }, extra));
					if (res && res.error) {
						reportError(action + ": " + res.error);
						return false;
					}
					await load(wsPath);
					return true;
				} catch (err) {
					reportError(action + " threw: " + String((err && err.message) || err));
					return false;
				}
			};
			const toggleCollapsed = (key, setter) => setter((s) => Object.assign({}, s, { [key]: !s[key] }));
			/** 在指定分类中新建会话：连接工作区空白会话（复用或新建）→ 归入分类 → 打开。 */
			const startInCategory = async (wsPath, workspaceId, categoryId) => {
				try {
					const sessionId = await connectWorkspace(workspaceId);
					await mutate(wsPath, "assign", { sessionId, categoryId });
					open(sessionId);
				} catch (err) {
					reportError("start in category threw: " + String((err && err.message) || err));
				}
			};
			const memberNodes = (store, wsIds, catId) => {
				const members = wsIds.filter((id) => {
					const cat = store.sessionCategory[id];
					return catId === null ? cat === void 0 : cat === catId;
				}).map((id) => byId[id]);
				members.sort(byRecency);
				return members.map((s) => sessionNode(s, descendants));
			};
			const sessionRows = (nodes) => nodes.map((node) => (0, react_jsx_runtime.jsx)(SessionNodeItem, {
				node,
				currentId: current,
				now,
				onOpen: open,
				...
				onArchive: onSessionArchive,
				drag: {
					start: () => setDragId(node.id),
					end: () => setDragId(null)
				},
				t
			}, node.id));
			const emptyNode = (0, react_jsx_runtime.jsx)("div", { className: WorkspaceBrowser_module_css_default.empty, children: t("empty.none") });
			const folderRow = (key, name, count, collapsedFlag, onClick, onDrop, ops, drag) => (0, react_jsx_runtime.jsx)("div", {
				className: clsx(
					"sc-folderRow",
					dragId !== null && "dragover",
					drag !== void 0 && drag.dragging === true && "sc-dragging"
				),
				role: "treeitem",
				"aria-expanded": !collapsedFlag,
				onClick,
				draggable: drag !== void 0 && drag.draggable === true,
				onDragStart: drag === void 0 ? void 0 : drag.onDragStart,
				onDragEnd: drag === void 0 ? void 0 : drag.onDragEnd,
				onDragOver: (e) => {
					if (dragId !== null) { e.preventDefault(); return; }
					if (drag !== void 0 && drag.onDragOver !== void 0) drag.onDragOver(e);
				},
				onDragLeave: (e) => {
					if (drag !== void 0 && drag.onDragLeave !== void 0) drag.onDragLeave(e);
				},
				onDrop: (e) => {
					e.preventDefault();
					const catId = e.dataTransfer.getData("application/x-dsh-category");
					if (catId !== "") {
						if (drag !== void 0 && drag.onCatDrop !== void 0) drag.onCatDrop(e);
						return;
					}
					const sid = e.dataTransfer.getData("text/plain");
					if (sid) { onDrop(sid); setDragId(null); }
				},
				children: [
					(0, react_jsx_runtime.jsx)("span", {
						className: clsx(Rows_module_css_default.slot, Rows_module_css_default.folder),
						children: collapsedFlag ? (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconFolderClose16, {}) : (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconFolderOpen16, {})
					}),
					(0, react_jsx_runtime.jsx)("span", {
						className: clsx(Rows_module_css_default.slot, Rows_module_css_default.chevron),
						children: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconTriangleRightFill14, { className: clsx(Rows_module_css_default.arrow, !collapsedFlag && Rows_module_css_default.arrowOpen) })
					}),
					(0, react_jsx_runtime.jsx)("span", { className: "sc-folderName", children: name }),
					(0, react_jsx_runtime.jsx)("span", { className: "sc-folderCount", children: String(count) }),
					(0, react_jsx_runtime.jsx)("span", { className: "sc-folderOps", children: ops })
				]
			});
			/**
			* 块级落点判定：悬停块上缘 30px = 插到该分类前（beforeId=自身）；
			* 下缘 30px = 插到其后（beforeId=下一个分类；最后一个分类→null=移到「未分类」前）。
			* 折叠态块很矮（<60px）时按中点二分，避免上下区域重叠。
			*/
			const boundaryBeforeId = (e, catId, nextId) => {
				const rect = e.currentTarget.getBoundingClientRect();
				const h = rect.height;
				if (h < 60) return e.clientY < rect.top + h / 2 ? catId : nextId;
				if (e.clientY < rect.top + 30) return catId;
				if (e.clientY > rect.bottom - 30) return nextId;
				return void 0;
			};
			return (0, react_jsx_runtime.jsxs)("div", {
				className: clsx(WorkspaceBrowser_module_css_default.treeBody, WorkspaceBrowser_module_css_default.wide),
				children: [
					error !== "" && (0, react_jsx_runtime.jsx)("div", { className: "sc-error", children: error }),
					(0, react_jsx_runtime.jsxs)("div", {
						className: WorkspaceBrowser_module_css_default.list,
						role: "tree",
						"aria-label": t("section.sessions"),
						children: workspaces.map((ws) => {
							const store = stores[ws.path] || { categories: [], sessionCategory: {} };
							const wsIds = ws.sessionIds.filter((id) => byId[id] !== void 0 && sessionVisible(byId[id], current, archived));
							const wsCollapsed = collapsedWs[ws.workspaceId] === true;
							const group = {
								key: ws.workspaceId,
								workspaceId: ws.workspaceId,
								cwd: ws.path,
								createdAt: Date.parse(ws.createdAt),
								label: ws.title,
								sessionCount: wsIds.length,
								expanded: !wsCollapsed,
								containsCurrent: current !== void 0 && ws.sessionIds.includes(current)
							};
							const children = [];
							children.push((0, react_jsx_runtime.jsx)(ProjectRowItem, {
								group,
								t,
								onToggle: () => toggleCollapsed(ws.workspaceId, setCollapsedWs),
								onCreate: () => startSession(ws.workspaceId),
								drag: void 0,
								actions: {
									rename: () => onRenameRequest(ws.workspaceId, ws.title),
									delete: () => onDeleteRequest(ws.workspaceId, ws.title)
								}
							}, ws.workspaceId + "-head"));
							if (!wsCollapsed) {
								if (creatingFor === ws.path) {
									const cancelCreate = () => { setCreatingFor(null); setNewName(""); };
										...
										children: [
											(0, react_jsx_runtime.jsx)("input", {
												...
												onKeyDown: async (e) => {
													...
													setCreatingFor(null);
													setNewName("");
													if (name !== "") await mutate(ws.path, "create", { name });
												}
											}),
											(0, react_jsx_runtime.jsx)("button", {
												type: "button",
												className: "sc-newcat-x",
												title: "取消",
												"aria-label": "取消",
									...
									children.push((0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: "sc-newcat",
										onClick: () => { setCreatingFor(ws.path); setNewName(""); },
										children: (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconPlusOutline16, {}), " 新建分类"] })
									}));
								}
								for (let i = 0; i < store.categories.length; i++) {
									const cat = store.categories[i];
									const nextId = store.categories[i + 1] === void 0 ? null : store.categories[i + 1].id;
									const catNodes = memberNodes(store, wsIds, cat.id);
									const catCollapsed = collapsed[cat.id] === true;
									const blockDrag = dragCat !== null && dragCat.wsPath === ws.path ? {
										openSlot: catMarker !== null && catMarker.beforeId === cat.id,
										onDragOver: (e) => {
											if (dragCat.catId === cat.id) { setCatMarker(null); return; }
											e.preventDefault();
											e.dataTransfer.dropEffect = "move";
											const beforeId = boundaryBeforeId(e, cat.id, nextId);
											setCatMarker(beforeId === void 0 ? null : { beforeId });
										},
										onDrop: (e) => {
											if (dragCat.catId === cat.id) return;
											const beforeId = boundaryBeforeId(e, cat.id, nextId);
											if (beforeId === void 0) return;
											setCatMarker(null);
											mutate(ws.path, "move", { categoryId: dragCat.catId, beforeCategoryId: beforeId }).then(() => setDragCat(null));
										}
									} : void 0;
									const rowDrag = {
										draggable: dragCat === null && dragId === null,
										dragging: dragCat !== null && dragCat.catId === cat.id,
										onDragStart: (e) => {
											e.dataTransfer.effectAllowed = "move";
											e.dataTransfer.setData("application/x-dsh-category", cat.id);
											setDragCat({ catId: cat.id, wsPath: ws.path });
										},
										onDragEnd: () => { setDragCat(null); setCatMarker(null); }
									};
									children.push((0, react_jsx_runtime.jsx)("div", {
										className: clsx("sc-folder", blockDrag !== void 0 && blockDrag.openSlot && "sc-openSlot"),
										onDragOver: blockDrag === void 0 ? void 0 : blockDrag.onDragOver,
										onDrop: blockDrag === void 0 ? void 0 : blockDrag.onDrop,
										children: [
											renaming === cat.id ? (0, react_jsx_runtime.jsx)("div", {
												className: "sc-newcat",
												children: (0, react_jsx_runtime.jsx)("input", {
													...
													onKeyDown: async (e) => {
														...
														setRenaming(null);
														if (name !== "") await mutate(ws.path, "rename", { categoryId: cat.id, name });
													}
												})
											}) : folderRow(
												cat.id,
												...
												catNodes.length,
												catCollapsed,
												() => toggleCollapsed(cat.id, setCollapsed),
												(sid) => mutate(ws.path, "assign", { sessionId: sid, categoryId: cat.id }),
												[
													(0, react_jsx_runtime.jsx)("button", { type: "button", className: Rows_module_css_default.iconButton, title: "重命名", onClick: (e) => { e.stopPropagation(); setRenaming(cat.id); }, children: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconEditOutline16, {}) }),
													(0, react_jsx_runtime.jsx)("button", { type: "button", className: Rows_module_css_default.iconButton, title: confirmDelete === cat.id ? "再点一次确认删除（会话回到未分类）" : "删除分类", onClick: (e) => { e.stopPropagation(); if (confirmDelete !== cat.id) { setConfirmDelete(cat.id); return; } setConfirmDelete(null); mutate(ws.path, "delete", { categoryId: cat.id }); }, children: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconTrashOutline16, {}) }),
													(0, react_jsx_runtime.jsx)("button", { type: "button", className: Rows_module_css_default.iconButton, title: "在该分类新建会话", onClick: (e) => { e.stopPropagation(); startInCategory(ws.path, ws.workspaceId, cat.id); }, children: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconPlusOutline16, {}) })
												],
												rowDrag
											),
											!catCollapsed && catNodes.length > 0 && sessionRows(catNodes),
											!catCollapsed && catNodes.length === 0 && emptyNode
										]
									}, cat.id));
								}
								const uncatKey = ws.workspaceId + "/__uncat__";
								const uncatNodes = memberNodes(store, wsIds, null);
								const uncatCollapsed = collapsed[uncatKey] === true;
								const ungroupedBlockDrag = dragCat !== null && dragCat.wsPath === ws.path ? {
									openSlot: catMarker !== null && catMarker.beforeId === null,
									onDragOver: (e) => {
										e.preventDefault();
										e.dataTransfer.dropEffect = "move";
										setCatMarker({ beforeId: null });
									},
									onDrop: () => {
										setCatMarker(null);
										mutate(ws.path, "move", { categoryId: dragCat.catId, beforeCategoryId: null }).then(() => setDragCat(null));
									}
								} : void 0;
								children.push((0, react_jsx_runtime.jsx)("div", {
									className: clsx("sc-folder", ungroupedBlockDrag !== void 0 && ungroupedBlockDrag.openSlot && "sc-openSlot"),
									onDragOver: ungroupedBlockDrag === void 0 ? void 0 : ungroupedBlockDrag.onDragOver,
									onDrop: ungroupedBlockDrag === void 0 ? void 0 : ungroupedBlockDrag.onDrop,
									children: [
										folderRow(
											uncatKey,
											"未分类",
											uncatNodes.length,
											uncatCollapsed,
											() => toggleCollapsed(uncatKey, setCollapsed),
											(sid) => mutate(ws.path, "assign", { sessionId: sid, categoryId: null }),
											[
												(0, react_jsx_runtime.jsx)("button", { type: "button", className: Rows_module_css_default.iconButton, title: "在未分类新建会话", onClick: (e) => { e.stopPropagation(); startInCategory(ws.path, ws.workspaceId, null); }, children: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconPlusOutline16, {}) })
											]
										),
										!uncatCollapsed && uncatNodes.length > 0 && sessionRows(uncatNodes),
										!uncatCollapsed && uncatNodes.length === 0 && emptyNode
									]
								}, uncatKey));
							}
		...
		//#endregion
		/** The workspace/session browsing region (replaces the official browser when the category layer is active). */
		function WorkspaceBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, connectWorkspace, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, searchSessions, searchResultLimit, useDirectoryFlow, renderSlot, t }) {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: wide && (normalizedQuery !== "" ? (0, react_jsx_runtime.jsx)(SearchResults, {
							...
							t
						}) : (0, react_jsx_runtime.jsx)(CategoryTree, {
							useSessions,
							onSessionRename,
							onSessionArchive,
							forkSession,
		...
		*/
		...
		class ScErrorBoundary extends react.Component {
			constructor(props) {
				super(props);
			...
			render() {
				if (this.state.error !== null) {
					return react.createElement("div", { style: { padding: 10, fontSize: 12, color: "#b3261e", whiteSpace: "pre-wrap", wordBreak: "break-all", fontFamily: "monospace" } }, "dsh-session-categories 渲染失败: " + String((this.state.error && this.state.error.stack) || this.state.error));
				}
		...
		function apply(ctx) {
			...
			try {
				...
				ctx.effect(() => {
					try {
						...
						// namespace "workspace" 已注册（官方 bundle 先加载），沿用现有字典。
					}
			...
			// 会话头部工作区选择器（conversation.hero.workspace）：该 slot 及子槽
			// conversation.hero.workspace.directoryFlow 由 conversation 主界面包（x6）
